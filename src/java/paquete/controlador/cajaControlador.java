...
public class cajaControlador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion.equalsIgnoreCase("apertura")) {
            writer.println(gson.toJson(dao.AperturadeCaja()));
        } else if (accion.equalsIgnoreCase("abrir")) {
            caja c = new caja();
            c.setSaldo_anterior(request.getParameter("saldo_anterior"));
            c.setSaldo_disponible(request.getParameter("saldo_disponible"));
            writer.println(dao.abrirCaja(c));
        } else if (accion.equalsIgnoreCase("reporte")) {
            JSONObject obj = new JSONObject();
            try {
                obj.put("data", dao.ReporteCaja());
